...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.96\linewidth]{figures/fig_qualitative.pdf}
\end{center}
...
seeds individually. Abstention is what removes false certainty: the same system
without it answers confidently on unresolvable scenes at $0.832$ and with it at
$0.000$, while committed accuracy rises from $0.722$ to $0.855$.
...
(Figure~\ref{fig:transfer}a). Nested selection, choosing hyperparameters on inner
folds and scoring only on outer folds never seen during selection, moves the result
by under $0.01$, and \textsc{XGBoost} and histogram boosting agree within $0.002$,
so the figures are not artefacts of tuning. On a single checkpoint, where every
method sees identical data, a head reading \emph{frozen} encoder features reaches
$\mathbf{0.879}$ against $0.858$ for ten hand-designed quantities and $0.819$ for
the strongest single feature: evidence of ambiguity is already present in the
representation and available to be decoded. Table~\ref{tab:supp-learned} gives the
full comparison.
