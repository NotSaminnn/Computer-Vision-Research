...
Table~\ref{tab:learned} answers both. Under leave-one-model-out the policy is
trained on eleven checkpoints and tested on the twelfth, which it has never seen,
and it exceeds the strongest single feature on $12$ of $12$. Nested selection moves
the result by under $0.01$ and two boosted families agree within $0.002$, so the
figures are not artefacts of tuning. On a single checkpoint, where every method
sees identical data, a head reading \emph{frozen} encoder features exceeds ten
hand-designed quantities: evidence of ambiguity is already present in the
representation and available to be decoded.
