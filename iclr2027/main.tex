...
registering absent evidence where one image stays consistent with every hypothesis. The contributions are as follows:
...
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_model_grid.pdf}
...
$9$ of $13$. Two further baselines place that gain in context. Holding the first frame of
each scene, which amounts to no choice at all, leaves the error at $0.0671$, a uniformly
random choice reaches $0.0591$, and a viewpoint chosen by the confidence of the predictor
reaches $0.0439$, against $0.0406$ for the best-explained observation. Selecting on the
evidence therefore improves on selecting by confidence, and the ordering across the four
rules follows the amount of evidence each rule consults. Real images carry no mechanism
annotation, and this experiment therefore measures error at the viewpoint that the rule
selects.
...
threshold taken from out-of-fold scores under folds split by scene. The curve stays
monotone across the whole range, from $61.3\%$ at $90\%$ coverage to $23.6\%$ at $30\%$, and
a deployment therefore trades answered inputs against failures at a rate the operator
chooses rather than at one fixed point.
...
view within a bounded action set licenses a commitment. The sequential design of experiments
\citep{chernoff1959sequential} already treats the choice of a next experiment as a decision
between rival hypotheses, and we borrow from that lineage the maximin criterion that optimal
design calls $T$-optimality
...
\paragraph{Limitations.} The criterion requires an executed action, the hypothesis family
covers planar optics alone, and real photographs provide a single stereo baseline.
Appendix~\ref{sec:supp-limits} states each limit in full, and
Appendix~\ref{sec:supp-extension} describes the route beyond planar optics.
